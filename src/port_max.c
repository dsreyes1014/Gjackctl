...
get_port_max ()
{
    ...
    if (config_lookup_string (&config, "gjackctl.server.port_max", &string) == CONFIG_FALSE)
    {
        gchar *value_copy;
        config_setting_t *group;
        config_setting_t *setting;

        g_print ("\'Port Max\' config option not available\n");
        g_print ("Creating config setting now...\n");

        value_copy = g_strdup ("512");
        group = config_lookup (&config, "gjackctl.server");
        setting = config_setting_add (group, "port_max", CONFIG_TYPE_STRING);
...
{
    gint i;
    static gchar *port_max[5] = {"128", "256", "512", "1024", "2048"};

    for (i = 1; i <= 5; i++)
    {
        if (g_strcmp0 (port_max[i - 1], gtk_entry_get_text (GTK_ENTRY (button))) == 0)
        {
            break;
        }
    }
           ...
           gpointer       user_data)
{
    ...
    for (i = 1; i <= 5; i++)
    {
        if ((gdouble) i == value)
        {
            gtk_entry_set_text (GTK_ENTRY (spin_button), port_max[i - 1]);
        }
    }
...
get_initial_value (const gchar *str)
{
    ...
    for (i = 1; i <= 5; i++)
    {
        if (g_strcmp0 (port_max[i - 1], str) == 0)
        {
            break;
        }
    }
...
port_max (GtkWidget *grid)
{
    ...
    adj = gtk_adjustment_new (get_initial_value (str), 1.0, 5.0, 1.0, 0, 0);
