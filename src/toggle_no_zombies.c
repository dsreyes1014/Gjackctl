...
get_no_zombies ()
{
	...
	if (config_lookup_bool (&config, "gjackctl.server.no_zombies", &value) == CONFIG_FALSE)
    {
        config_setting_t *group;
        config_setting_t *setting;

        g_print ("\'No Zombies\' config option not available\n");
        g_print ("Creating config setting now...\n");

        group = config_lookup (&config, "gjackctl.server");

        setting = config_setting_add (group, "no_zombies", CONFIG_TYPE_BOOL);
...
toggle_no_zombies (GtkWidget *grid)
{
    ...
    cbutton = gtk_check_button_new_with_label ("No Zombies");
