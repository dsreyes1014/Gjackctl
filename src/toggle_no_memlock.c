...
get_no_memlock ()
{
	...
	if (config_lookup_bool (&config, "gjackctl.server.no_memlock", &value) == CONFIG_FALSE)
    {
        config_setting_t *group;
        config_setting_t *setting;

        g_print ("\'No Memlock\' config option not available\n");
        g_print ("Creating config setting now...\n");

        group = config_lookup (&config, "gjackctl.server");

        setting = config_setting_add (group, "no_memlock", CONFIG_TYPE_BOOL);
...
toggle_no_memlock (GtkWidget *grid)
{
    ...
    cbutton = gtk_check_button_new_with_label ("No Memlock");
